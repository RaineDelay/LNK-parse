...
def extract(filepath):
    with open(filepath, "rb") as fd:
        ...
        set_attrs = get_attributes(fd.read(4))              # same as flags but with the file's attributes instead
...
def get_flags(byte_arr):
    ...
    set_flags = [flag for i, flag in enumerate(flag_list) if (byte_arr[i // 8] & (1 << (7 - (i % 8)))) > 0]
    print(set_flags)
    return set_flags
